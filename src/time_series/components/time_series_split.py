...
class DataSplit:
    def __init__(self, split_on_variable : str):        
        ...
        # * I use the original dataset since I am to clean it in the splitting process
    ...
    def initiate_data_split(self):
        try:
            ...
            for n, sub in enumerate(subgroups):
                ...
                df_tmp = df.loc[df[self.split_on_variable] == sub].copy()
                ...
                if len(missing_value_cols) > 0:
                    for col in missing_value_cols:
                        df_tmp[col].ffill(inplace = True)
                        
                """# Standardize the STORE_ID variable
                for n, s in enumerate(df_tmp.loc[:, 'STORE_ID'].str.replace('TORE', '0')):
                    s = s[1:]
                    s = 'S' + s
                    df_tmp.loc[n, 'STORE_ID'] = s"""
                # -----------------------------
